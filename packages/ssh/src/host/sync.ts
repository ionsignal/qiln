...
import type { SshSyncLogger } from './types'

/**
 * Non-authoritative SSH-to-Worker branch authorized-key dispatcher.
 *
 * This service derives the current canonical key set only from active SSH
 ...
 * changes SSH grant, fence, ticket, or relay authority.
 */
export class SshAuthorizedKeysSyncDispatcher {
  ...
  constructor(
    private readonly persistence: SshPersistence,
   ...
   * policy transaction has committed.
   *
  ...
  public scheduleBranch(ownerId: string, capsuleId: string, branchId: string): void {
    ...
    const delivery = this.dispatchBranch(ownerId, capsuleId, branchId)
      .catch((error: unknown) => {
        this.logger.warn(
          {
            ownerId,
  ...
  /**
   * The runtime drains policy calls first so committed mutations have already
   * scheduled their detached deliveries before this dispatcher stops.
   */
  ...
  private async dispatchBranch(ownerId: string, capsuleId: string, branchId: string): Promise<void> {
    ...
    const { sshBranchGrants, sshPublicKeys } = tables
    const keys = await db
      .select({
