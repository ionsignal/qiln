...
export class SshBranchGrantService {
  ...
  public async bind(adminUserId: string, publicKeyId: string, branchId: string): Promise<SshBranchGrantSummary> {
    const { users, sshPublicKeys, capsuleBranches, capsules, sshBranchAccess, sshBranchGrants } = this.tables
  ...
  public async revoke(adminUserId: string, grantId: string): Promise<SshBranchGrantSummary> {
    const { users, sshBranchGrants, sshPublicKeys, capsuleBranches, sshTickets } = this.tables
  ...
  public async listAll(adminUserId: string): Promise<SshBranchGrantSummary[]> {
    const { users, sshBranchGrants, sshPublicKeys, capsuleBranches } = this.tables
  ...
  public async generateOpenSshConfig(userId: string, publicKeyId: string): Promise<SshOpenSshConfigOutput> {
    const { sshBranchGrants, sshPublicKeys, capsuleBranches, capsules, sshBranchAccess } = this.tables
  ...
  private summary(grant: GrantRow, key: PublicKeyRow, branchName: string): SshBranchGrantSummary {
