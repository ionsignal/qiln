...
type PublicKeyRow = SshPersistence['tables']['sshPublicKeys']['$inferSelect']
type GrantRow = SshPersistence['tables']['sshBranchGrants']['$inferSelect']

interface SshAuthorizedBinding {
  userId: string
  capsuleId: string
  branchId: string
  registeredKey: PublicKeyRow
  grant: GrantRow
}
...
export class SshTicketService {
  ...
  public async checkEligibility(key: SshCanonicalPublicKey): Promise<SshGatewayKeyEligibilityOutput> {
    const { sshPublicKeys, sshBranchGrants, sshBranchAccess, capsuleBranches, capsules } = this.tables
  ...
  public async redeem(
    ...
    const { sshTickets, sshPublicKeys, sshBranchGrants, sshRelays } = this.tables
  ...
  public async activate(relayId: string, gatewayInstanceId: string): Promise<SshRelayActivationOutput> {
    const { sshRelays, sshTickets, sshPublicKeys, sshBranchGrants } = this.tables
    const parsedGatewayInstanceId = SshGatewayInstanceIdSchema.parse(gatewayInstanceId)
    return await this.db.transaction(async tx => {
      const [relay] = await tx
        .select()
  ...
  private async lockEligibleBinding(tx: SshDatabaseTransaction, key: SshCanonicalPublicKey) {
    const { sshPublicKeys, sshBranchGrants } = this.tables
    const [registeredKey] = await tx
      .select()
      .from(sshPublicKeys)
      .where(
  ...
  private async lockAndValidateDestination(tx: SshDatabaseTransaction, binding: SshAuthorizedBinding) {
    const { capsuleBranches, capsules, sshBranchAccess } = this.tables
