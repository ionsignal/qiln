 ...
 * Replaces {{ path.to.variable }} with the corresponding primitive value from
 * the data object.
 *
...
export function interpolate(template: string, data: Record<string, unknown>): string {
  return template.replace(/\{\{\s*([\w.-]+)\s*\}\}/g, (_match, variablePath: string) => {
    ...
    for (const key of keys) {
      if (!isRecord(current)) {
        throw new IncusError(
          `Interpolation failed: Missing or invalid variable path '${variablePath}'`,
          'VALIDATION_ERROR',
        )
      }
      current = current[key]
    ...
    if (!isInterpolablePrimitive(current)) {
      throw new IncusError(
        `Interpolation failed: Resolved value for '${variablePath}' is invalid (must be a primitive)`,
        'VALIDATION_ERROR',
      )
    }
