...
> {
  ...
  public async build(
    ...
    const operations = await tx.select().from(t.capsuleOperations)
      .where(eq(t.capsuleOperations.capsuleId, operation.capsuleId))
      .orderBy(asc(t.capsuleOperations.id)).for('update')
    ...
    const creates = await tx.select().from(t.capsuleCreateOperations)
      .where(or(
        inArray(t.capsuleCreateOperations.operationId, operationIds),
        inArray(t.capsuleCreateOperations.rootBranchId, branchIds),
      )).for('update')
    const forks = await tx.select().from(t.capsuleForkOperations)
      .where(or(
    ...
    const resources = await tx.select().from(t.capsuleBranchResources)
      .where(or(
        inArray(t.capsuleBranchResources.branchId, branchIds),
        inArray(t.capsuleBranchResources.createdByOperationId, operationIds),
      )).orderBy(asc(t.capsuleBranchResources.id)).for('update')
    for (const proof of proofs.values()) {
      verifyResources(proof, resources.filter(resource =>
        resource.branchId === proof.branch.id || resource.createdByOperationId === proof.origin.id,
      ))
    }
    if (resources.some(resource =>
      ![...proofs.values()].some(proof =>
        resource.branchId === proof.branch.id || resource.createdByOperationId === proof.origin.id,
      ),
    )) {
      throw new IncusError('Destroy found unattributable branch resource accounting.', 'CONFLICT')
    }
    const snapshots = await tx.select().from(t.capsuleSnapshots)
      .where(eq(t.capsuleSnapshots.capsuleId, operation.capsuleId))
      .orderBy(asc(t.capsuleSnapshots.id)).for('update')
    const snapshotIds = snapshots.map(snapshot => snapshot.id)
    const captures = await tx.select().from(t.capsuleSnapshotCreateOperations)
      .where(or(
    ...
    const captureResources = captureIds.length === 0 ? [] : await tx.select().from(t.capsuleSnapshotCreateResources)
      .where(inArray(t.capsuleSnapshotCreateResources.operationId, captureIds)).for('update')
    const references = snapshotIds.length === 0 ? [] : await tx.select().from(t.capsuleSnapshotResourceReferences)
      .where(inArray(t.capsuleSnapshotResourceReferences.snapshotId, snapshotIds)).for('update')
    ...
    const aliases = await tx.select().from(t.capsuleRouteAliases)
      .where(eq(t.capsuleRouteAliases.capsuleId, operation.capsuleId)).for('update')
    const aliasIds = aliases.map(alias => alias.id)
    const revisions = await tx.select().from(t.capsuleRouteRevisions)
      .where(or(
    ...
    const routeOperations = await tx.select().from(t.capsuleRouteOperations)
      .where(or(
    ...
    const providers = await tx.select().from(t.capsuleRouteProviderApplications)
      .where(inArray(t.capsuleRouteProviderApplications.operationId, operationIds)).for('update')
    const heads = aliasIds.length === 0 ? [] : await tx.select().from(t.capsuleRouteHeads)
      .where(inArray(t.capsuleRouteHeads.aliasId, aliasIds)).for('update')
    ...
    const storage = orderedBranches.reverse().flatMap(branchId => [
      ...snapshotPlans.filter(resource => resource.proof.source === 'snapshot' && resource.proof.branchId === branchId),
