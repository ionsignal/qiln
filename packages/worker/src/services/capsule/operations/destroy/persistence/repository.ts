...
> {
    ...
    const accept = async (): Promise<DestroyCapsuleRepositoryResult> => {
      return await this.persistence.db.transaction(async tx => {
        ...
        const [active] = await tx
          .select({ id: t.capsuleOperations.id })
  ...
  /**
   * Planning and target persistence precede retirement and all external work. A
   * crash between these transactions is classified, never resumed.
   */
  public async prepare(operationId: string): Promise<DestroyExecution> {
    ...
    const state = await this.persistence.db.transaction(async tx => {
      ...
      const now = new Date()
      await tx
        .update(t.capsuleSnapshots)
        .set({
          retiredAt: now,
          retiredByOperationId: operationId,
        })
        .where(and(eq(t.capsuleSnapshots.capsuleId, locked.capsule.id), isNull(t.capsuleSnapshots.retiredAt)))
      await tx
        .update(t.capsuleBranchPreviews)
        .set({
  ...
  /**
   * Clears derived preview state and alias heads only after every planned route
   * obligation has a confirmed terminal outcome.
   */
  public async withdrawn(operationId: string): Promise<void> {
    await this.persistence.db.transaction(async tx => {
      ...
      if (
        state.targets.some(
          target => target.target.kind === 'route' && target.status !== 'absent' && target.status !== 'deleted',
        )
      ) {
      ...
      const aliases = await tx
        .select({ id: t.capsuleRouteAliases.id })
        .from(t.capsuleRouteAliases)
        .where(eq(t.capsuleRouteAliases.capsuleId, locked.capsule.id))
        .for('update')
      if (aliases.length > 0) {
        await tx.delete(t.capsuleRouteHeads).where(
          inArray(
            t.capsuleRouteHeads.aliasId,
  ...
  public async complete(operationId: string): Promise<DestroyCapsuleTerminalResult> {
    return await this.persistence.db.transaction(async tx => {
      ...
      if (
        state.targets.some(target => target.status === 'deleted') &&
      ...
      const [usableSnapshot] = await tx
        .select({ id: t.capsuleSnapshots.id })
      ...
      const now = new Date()
      const [operation] = await tx
        .update(t.capsuleOperations)
        .set({ status: 'completed', completedAt: now, updatedAt: now })
        .where(and(eq(t.capsuleOperations.id, operationId), eq(t.capsuleOperations.status, 'running')))
        .returning()
      const [capsule] = await tx
        .update(t.capsules)
        .set({ lifecycleStatus: 'destroyed', destroyedAt: now, updatedAt: now })
        .where(eq(t.capsules.id, locked.capsule.id))
        .returning()
      const branches = await tx
        .update(t.capsuleBranches)
        .set({ status: 'destroyed', runtimeIp: null, updatedAt: now })
        .where(eq(t.capsuleBranches.capsuleId, locked.capsule.id))
        .returning()
  ...
  private async classify(
    ...
    return await this.persistence.db.transaction(async tx => {
      ...
      const t = this.persistence.tables
      const [plan] = await tx
        .select({ operationId: t.capsuleDestroyOperations.operationId })
        .from(t.capsuleDestroyOperations)
        .where(eq(t.capsuleDestroyOperations.operationId, operationId))
        .limit(1)
      const restore =
      ...
      await tx
        .update(t.capsuleDestroyResources)
      ...
      const [operation] = await tx
        ...
        .where(eq(t.capsuleOperations.id, operationId))
        .returning()
      ...
      const mutable = locked.branches.filter(
        branch => branch.ownerId === locked.operation.ownerId && branch.status !== 'destroyed',
      )
      if (mutable.length > 0) {
        await tx
          .update(t.capsuleBranches)
  ...
  private assertAccess(state: DestroyTargetState): void {
    ...
    if (
      ...
      throw new IncusError(
        'Destroy requires this attempt’s complete SSH revocation and relay closure evidence.',
        'CONFLICT',
      )
    }
  }
